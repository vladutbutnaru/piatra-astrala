 ...
 */
public class ChestArmorController {
    ...
    public static ChestArmor getById(int id) {
        ChestArmor m = new ChestArmor();

        PreparedStatement stmt;
        ResultSet rs;

        try {
            stmt = conn.prepareStatement("SELECT * FROM Item_Chest WHERE ID = ?");
            stmt.setInt(1, id);
