 ...
 */
public class HandAccessoryController {
    ...
    public static HandAccessory getById(int id) {
        HandAccessory m = new HandAccessory();

        PreparedStatement stmt;
        ResultSet rs;

        try {
            stmt = conn.prepareStatement("SELECT * FROM Item_HandAccessories WHERE ID = ?");
            stmt.setInt(1, id);
