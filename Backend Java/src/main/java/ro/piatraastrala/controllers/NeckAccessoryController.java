...
import ro.piatraastrala.entities.NeckAccessory;
 ...
 */
public class NeckAccessoryController {
    ...
    public static NeckAccessory getById(int id) {
        NeckAccessory m = new NeckAccessory();

        PreparedStatement stmt;
        ResultSet rs;

        try {
            stmt = conn.prepareStatement("SELECT * FROM Item_Neck WHERE ID = ?");
            stmt.setInt(1, id);
