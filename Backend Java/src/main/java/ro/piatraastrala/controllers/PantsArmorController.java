...
import ro.piatraastrala.entities.PantsArmor;
 ...
 */
public class PantsArmorController {
    ...
    public static PantsArmor getById(int id) {
        PantsArmor m = new PantsArmor();

        PreparedStatement stmt;
        ResultSet rs;

        try {
            stmt = conn.prepareStatement("SELECT * FROM Item_Pants WHERE ID = ?");
            stmt.setInt(1, id);
