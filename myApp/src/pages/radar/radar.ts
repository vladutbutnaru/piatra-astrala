...
getMissionsForNpc(npcID:any){
    ...
    for(let i = 0; i<this.missionList.length; i++){
        ...
        this.missionListFront += '<div class="npcMission" id="mission_'+this.missionList[i].id+'">'+this.missionList[i].title+'</div>';
...
acceptMission(missionID:any){
    ...
    this.http.post("http://192.168.1.172:8080/missions/v1/acceptmission", data, options)
      .subscribe(data => {
   this.navCtrl.push(MissionsPage, {user: this.user, mission: missionID });
